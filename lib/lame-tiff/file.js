...
import { fieldTagNames, fieldTypeMap } from './constants.js';
...
export async function parse(filename) {
  const fh = await open(filename, constants.O_RDONLY);
...
export async function header(fh) {
...
export async function ifd({ fh, littleEndian, ifdOffset }) {
